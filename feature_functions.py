...
def poss_title(fr):
    ...
    """
    result = _is_possessive_pronoun(fr.i_token) and (
        _is_profession(fr.j_token) or _is_official(fr.j_token) or _is_title(fr.j_token))
...
def per_title(fr):
    ...
    """
    result = fr.i_entity_type == 'PER' and (
        _is_profession(fr.j_token) or _is_official(fr.j_token) or _is_title(fr.j_token))
...
def nnp_title(fr):
    ...
    i_pos = POS_SENTENCES[fr.article][fr.i_sentence][fr.i_offset_begin][1]
    j_pos = POS_SENTENCES[fr.article][fr.j_sentence][fr.j_offset_begin][1]
    result = (i_pos.startswith("NNP") and (
        _is_profession(fr.j_token) or _is_official(fr.j_token) or _is_title(fr.j_token))) or \
             (j_pos.startswith("NNP") and (
                 _is_profession(fr.i_token) or _is_official(fr.i_token) or _is_title(fr.i_token)))
...
def et1_dw1(fr):
    ...
    dep_list = [dep_word for (dep_index, dep_word), (gov_index, gov_word, dep_type) in et1_dependencies.items() if
                int(fr.i_offset_end) == gov_index]
...
def et2_dw2(fr):
    ...
    dep_list = [dep_word for (dep_index, dep_word), (gov_index, gov_word, dep_type) in et2_dependencies.items() if
                int(fr.j_offset_end) == gov_index]
